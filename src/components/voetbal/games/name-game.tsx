...
import { VOETBAL_ROUND_SECONDS } from "@/lib/voetbal/config";
...
export function NameGame({
  ...
  async function onGuess(e: React.FormEvent) {
    ...
    toast.success(
      result.coins > 0
        ? `${result.name} · ${copy.voetbal.coinsGained(result.coins)}`
        : `${result.name} · ${copy.voetbal.capped}`,
    );
